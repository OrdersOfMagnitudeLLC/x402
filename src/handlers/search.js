...
function readStub(routePath) {
  const stubPath = path.join(__dirname, '../endpoints/search', `${routePath}.json`);
...
async function handleSearchRequest(req, res) {
  // Extract route path from URL (everything after /search/)
  const routePath = req.originalRoutePath || req.path.replace('/search/', '');
