...
function readStub(routePath) {
  const stubPath = path.join(__dirname, '../endpoints/temporal', `${routePath}.json`);
  const stubContent = fs.readFileSync(stubPath, 'utf8');
  return JSON.parse(stubContent);
}

// Generic temporal handler
async function handleTemporalRequest(req, res) {
  // Extract route path from URL (everything after /temporal/)
  const routePath = req.originalRoutePath || req.path.replace('/temporal/', '');
  ...
  try {
    ...
    console.error('Temporal handler error:', error);
    return res.status(502).json({ error: 'upstream_unavailable' });
  }
}

module.exports = { handleTemporalRequest };
