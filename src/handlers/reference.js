...
function readStub(routePath) {
  const stubPath = path.join(__dirname, '../endpoints/reference', `${routePath}.json`);
  const stubContent = fs.readFileSync(stubPath, 'utf8');
  return JSON.parse(stubContent);
}

// Generic reference handler
async function handleReferenceRequest(req, res) {
  // Extract route path from URL (everything after /reference/)
  const routePath = req.originalRoutePath || req.path.replace('/reference/', '');
  ...
  try {
    ...
    console.error('Reference handler error:', error);
    return res.status(502).json({ error: 'upstream_unavailable' });
  }
}

module.exports = { handleReferenceRequest };
